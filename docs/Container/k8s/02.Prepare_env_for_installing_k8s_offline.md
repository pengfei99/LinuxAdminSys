...
As our scenario is offline installation, so we need to have a private debian package repo. Suppose we have a 
debian package repo is built by using aptly. It has all the basic packages of 
debian 11, k8s-main(kubeadm, kubelet, etc.), containerd, and docker. The url of this server is `deb.casd.local`
